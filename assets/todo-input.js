...
const TodoInput = {
  ...
  <div>
    ...
    -->
    <span>内容: <input v-model="content" type="text"></span>
    <span>締め切り: <input v-model="date" type="date"><input v-model="time" type="time"></span>
    <!--
    @click により、クリックしたときの処理が登録できる
    ここでは、事前に Controller に作成した onRegister 関数を呼び出している
    -->
    <button @click="onRegister">登録</button>
  </div>
  `,
};
